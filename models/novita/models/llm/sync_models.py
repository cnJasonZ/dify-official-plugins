...
def create_yaml_template(model_id: str, api_model: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Add tool-call feature if function-calling is in api_model features
    if "features" in api_model and isinstance(api_model["features"], list) and "function-calling" in api_model["features"]:
...
def sync_yaml_files(yaml_dir: str):
    ...
    for model_id, api_model in api_models.items():
        if model_id in existing_files:
            ...
            has_function_calling = "features" in api_model and isinstance(api_model["features"], list) and "function-calling" in api_model["features"]
            has_tool_call = "features" in yaml_data and isinstance(yaml_data["features"], list) and "tool-call" in yaml_data["features"]
            
            if has_function_calling and not has_tool_call:
                yaml_data["features"].append("tool-call")
                changes.append("Added 'tool-call' to features")
            elif not has_function_calling and has_tool_call:
                yaml_data["features"].remove("tool-call")
                changes.append("Removed 'tool-call' from features")
